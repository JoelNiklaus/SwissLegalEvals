...
11. **LEXam MCQ option order is derived from the question.** lighteval shuffles the answer choices from a single module-level RNG, so a question's letters depend on how many questions were shuffled before it in that process. Scoring cached generations in a later process then re-rolled the letters and marked correct answers wrong, which invalidated the MCQ scores of every run that resumed from a cache. The order is now seeded from the choice texts, so it is identical in every process. Use `scripts/check_mcq_integrity.py` to confirm a run scored the prompts it generated from.
12. **API calls have a request timeout.** lighteval's inference-providers client defaults to no timeout, so a stream the provider stops feeding blocks forever. Runs default to 1800 s, overridable per model with `timeout` in `configs/models.yaml`. Note that changing it alters the model-config hash, so a model's existing sample cache is not reused.
