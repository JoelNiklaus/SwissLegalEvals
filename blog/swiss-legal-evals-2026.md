...
One pairing inside that tier is still curious: Kimi K3 (57.0) lands below its own predecessor K2.6 (58.6) despite carrying nearly three times the total parameters (2.8T versus 1T). The composite gap is inside the noise band, but the task-level pattern is not random — K3 gives up 5.7 points on LEXam open questions and 3.5 on MCQ while gaining slightly on translation and summarization, the profile you would expect from a release tuned hard for coding and agentic work, where the training signal rewards tool use and long-horizon execution rather than exam-style legal knowledge.
...
The abstention rates explain why, and they run backwards. Every model that abstains meaningfully at four options does so far less at sixteen, exactly where its chance of guessing right falls from one in four to one in sixteen. OLMo 3.1 32B Think is the most willing to decline, on 36.8% of four-option questions but only 3.4% at sixteen; LFM2.5 8B goes from 24.3% to 3.6%, DeepSeek V4 Pro from 7.3% to 0.9%. Llama 3.3 70B never abstains at all, and the two accuracy leaders barely do: Inkling declines on 0.2% of four-option questions and Gemma on 0.9%, and at sixteen they effectively never do (0.1% and 0.0%). Because abstention is this rare on the wide set, `idk_score` at 16 options is close to twice the accuracy minus 100 and barely reorders the field; what it adds is the level, not the ranking.
